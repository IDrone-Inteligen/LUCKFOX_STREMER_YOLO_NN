...
int main(int argc, char *argv[])
{
	...
	// cv::Video writer;
	// writer.open("output.avi", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 10, cv::Size(frame_width, frame_height), true);
    ...
    int fd = open("output.h264", O_WRONLY | O_CREAT, 0666);
	...
	while (!quit)
	{
		...
		if (s32Ret == RK_SUCCESS)
		{
			// void *data = RK_MPI_MB_Handle2VirAddr(h264_frame.stVFrame.pMbBlk);
			// opencv
			// cv::Mat frame_s(height, width, CV_8UC3, data);
			// cv::Mat frame640;
			...
			if (g_rtsplive && g_rtsp_session)
			{
				...
				write(fd, pData, stFrame.pstPack->u32Len);

				// memcpy(pData, frame.data, width * height * 3);
